<template>
    <div>
        <div id = "firstGrid">
            <div id = "hbTable">
                <div id = "hTable">
                    <table id = "headTable" :width = "tHeadWidth">
                        <tr>
                            <td v-for="(nameRow, iKey, indNam) in tableData.info" :width = "nameRow.sizeProc">{{ nameRow.description || nameRow.name }} {{ nameRow.sizeProc }}</td>
                        </tr>
                        <tr v-show="filterVisible" >
                            <!-- <td  v-for="nameRow in tableData.info"> -->
                            <td  v-for="nameRow in tableData.info" :width = "nameRow.sizeProc">
                            <input type = "text" width = "10" :placeholder=" nameRow.description || nameRow.name ">
                            </td>
                        </tr>
                    </table>
                </div>
                <div id = "bTable">
                    <table id = "cTable">
                        <tr id  = "cLine" v-for="(lineContent, indContent) in tableData.content" :key="lineContent.id" @mouseover="markBack(indContent)" @mouseout="cleanMarkBack" :class = "(markTrmMouse && markRow===indContent) ? 'trBack' : '' ">
                            <td v-for="(cell, cKey, indName) in lineContent" :width = "tableData.info[cKey].sizeProc"> {{ cell }} {{ tableData.info[cKey].sizeProc }} {{ indName }}</td>
                        </tr>
                    </table>
                </div>
            </div>
        ...
        methods:{
            ...
            async calcSizeHead(){
                ...
                let allWidth = 0,
                    ...
                    indKey;
                this.tBodyWidth = document.getElementById("bTable").clientWidth;
                // this.tBodyWidth = document.getElementById("bTable").scrollWidth;
                this.tHeadWidth = this.tBodyWidth;
                alert("Body clientWidth: "+ document.getElementById("bTable").clientWidth + "\n"
                        + "Body scrollWidth: "+ document.getElementById("bTable").scrollWidth);
                // alert("!Head: "+ this.tHeadWidth + " Body: "+this.tBodyWidth);
            ...
            async calcWidthProc(maxWidth, arrHead){
                ...
                // if (maxWidth > this.tBodyWidthh) {
                    ...
                    // console.log("=> width clientWidth %: ", document.body.clientWidth);
                    console.log("=> width clientWidth %: ", this.tBodyWidthh);
                    for (indArrHead = 0; indArrHead < arrHead.length; indArrHead++) {
                        this.tableData.info[arrHead[indArrHead].name].sizeProc = String((arrHead[indArrHead].size)/maxWidth*this.tBodyWidth);
